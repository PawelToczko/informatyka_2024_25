...
void loadLeaderboardFromFile(const std::string& filename) {
    ...
    if (!file.is_open()) {
        return; // Jeœli plik nie istnieje, pomiñ
    }
...
void showLeaderboard(sf::RenderWindow& window, sf::Font& font) {
    ...
    int yOffset = 100;
    // Odwrócenie iteracji po leaderboardzie, ¿eby najwy¿szy wynik by³ na górze
...
private:
    ...
    // Flaga œledz¹ca, czy klawisz spacji zosta³ zwolniony
    ...
    void loadResources() {
        ...
        if (!font.loadFromFile("arial.ttf")) {
            throw std::runtime_error("Nie uda³o siê wczytaæ czcionki");
        }
        ...
        if (!playerTexture.loadFromFile("player.png") ||
            ...
            throw std::runtime_error("Nie uda³o siê wczytaæ tekstur");
        }
    ...
    void initializeHealthSprites() {
        healthSprites.clear(); // Usuñ poprzednie sprite'y
    ...
    void updateHealthSprites() {
        if (health < healthSprites.size()) {
            healthSprites.pop_back(); // Usuniêcie ostatniego sprite'a zdrowia
        }
    }
    ...
    void updateFallingHearts() {
        ...
        for (auto& heart : fallingHearts) {
            heart.move(0, 0.05f); // Prêdkoœæ spadania serca
        }

        // Usuwanie serc, które spad³y poza ekran
        ...
        for (auto it = fallingHearts.begin(); it != fallingHearts.end();) {
            if (it->getGlobalBounds().intersects(player.getGlobalBounds())) {
                it = fallingHearts.erase(it); // Usuniêcie serca po zebraniu
                if (health < 3) {
                    health++; // Zwiêkszenie zdrowia
                    initializeHealthSprites(); // Aktualizacja wyœwietlania serc
                }
            }
            else {
                ++it;
    ...
    void updateGame() {
        ...
        for (auto& bullet : bullets) {
            for (auto it = enemies.begin(); it != enemies.end();) {
                if (bullet.getGlobalBounds().intersects(it->getGlobalBounds())) {
                    ...
                    it = enemies.erase(it); // Usuniêcie przeciwnika
                    score += 10;
                }
                else {
                    ++it;
        ...
        // Generowanie pocisków przeciwników z ograniczon¹ czêstotliwoœci¹
        ...
        for (auto it = enemyBullets.begin(); it != enemyBullets.end();) {
            if (it->getGlobalBounds().intersects(player.getGlobalBounds())) {
                ...
                if (health <= 0) {
                    ...
                    addToLeaderboard("Gracz", score); // Zmieñ "Gracz" na aktualn¹ nazwê gracza, jeœli dodasz jej wprowadzanie
        ...
        // Aktualizacja wyœwietlanych danych
    ...
    void loadGame() {
        ...
        while (std::getline(saveFile, line)) {
            if (line == "END") break; // Detekcja koñca sekcji wrogów
            std::istringstream iss(line);
            float x, y;
            if (iss >> x >> y) {
                sf::Sprite enemy(enemyTexture);
    ...
    int showMainMenu() {
        int selection = 0; // 0: Wczytaj grê, 1: Nowa gra, 2: Leaderboard
        ...
        return -1; // Domyœlny zwrot w przypadku zamkniêcia
    }
    ...
    SpaceInvaders() : window(sf::VideoMode(800, 600), "Space Invaders") {
        ...
        while (true) {
            ...
            if (menuChoice == 0) {
                ...
                showLeaderboard(window, font); // Wyœwietl leaderboard
            }
            else {
                window.close(); // Zamkniêcie gry w przypadku wyboru -1
                break;
            }
        }
    }
